...
from inference_bass import separate_bass

def evaluate_model(
    model,
    device,
    test_dir: str = 'musdb18/test',
    output_dir: str = 'evaluation_bass_results',
    log_dir: Optional[str] = 'logs',
    model_tag: str = 'unet_bass',
    limit: Optional[int] = None,
    save_comparison: bool = False,
    skip_existing: bool = True,
):
    """Evaluate bass separation across MUSDB18 test set and log results."""
    ...
    if log_dir:
        ...
        csv_path = os.path.join(log_dir, f'eval_bass_{model_tag}_{timestamp}.csv')
        json_path = os.path.join(log_dir, f'eval_bass_summary_{model_tag}_{timestamp}.json')
    ...
    for i, test_file in enumerate(test_files):
        ...
        output_filename = os.path.basename(test_file).replace('.stem.mp4', '_bass.wav')
        ...
        if skip_existing and os.path.exists(output_path):
            ...
            separate_bass(model, device, test_file, output_dir=output_dir)
            processing_time = time.time() - start_time

        try:
            separated_bass, sr = librosa.load(output_path, sr=None)
            duration = float(len(separated_bass) / sr) if sr else None
            rms = float(np.sqrt(np.mean(separated_bass**2))) if len(separated_bass) else None
            peak = float(np.max(np.abs(separated_bass))) if len(separated_bass) else None
            ...
            if save_comparison:
                try:
                    ...
                    if len(stems) > 2:
                        real_bass = stems[2]
                        ref = np.mean(real_bass, axis=1) if real_bass.ndim > 1 else real_bass
                        min_len = min(len(separated_bass), len(ref))
                        if min_len > 0:
                            sdr = float(calculate_sdr(ref[:min_len], separated_bass[:min_len]))
    ...
    if log_dir:
        try:
            ...
            with open(os.path.join(log_dir, f'eval_bass_summary_{model_tag}_{timestamp}.json'), 'w') as f:
                _json.dump({'summary': summary, 'results': results}, f, indent=2)
            print(f"Logs written: {csv_path}")
            print(f"Summary written: {os.path.join(log_dir, f'eval_bass_summary_{model_tag}_{timestamp}.json')}")
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate bass model over MUSDB18 test set and log results.")
    parser.add_argument('--test-dir', type=str, default='musdb18/test', help='Directory containing MUSDB18 test .stem.mp4 files')
    parser.add_argument('--output-dir', type=str, default='evaluation_bass_results', help='Directory to write separated outputs')
    parser.add_argument('--log-dir', type=str, default='logs', help='Directory to write CSV/JSON logs (set empty to disable)')
    parser.add_argument('--model-tag', type=str, default='unet_bass', help='Identifier for the evaluated model (used in logs)')
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoints/unet_bass.pth', help='Path to model checkpoint')
    ...
    try:
        ...
        print(f"Bass model loaded from {args.checkpoint}")
        model.eval()
    except Exception as e:
