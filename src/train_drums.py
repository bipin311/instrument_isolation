...
class MUSDBDrumsDataset(Dataset):
    """
    A PyTorch Dataset to load mixture and drums stems from the MUSDB18 dataset.
    """
    def __init__(self, root_dir, duration=2, samples_per_track=16):
        ...
        print(f"Found {len(self.track_paths)} tracks for drums training.")
    ...
    def __getitem__(self, idx):
        ...
        # Load mixture (stem 0) and drums (stem 1)
        stems, rate = stempeg.read_stems(track_path, stem_id=[0, 1])
        mixture_stereo = stems[0]
        drums_stereo = stems[1]

        # Convert to mono
        mixture_mono = np.mean(mixture_stereo, axis=1)
        drums_mono = np.mean(drums_stereo, axis=1)
        ...
        if len(mixture_mono) > segment_samples:
            ...
            drums_mono = drums_mono[start_sample:end_sample]

        # Compute STFT
        mixture_stft = librosa.stft(mixture_mono, n_fft=2048, hop_length=512)
        drums_stft = librosa.stft(drums_mono, n_fft=2048, hop_length=512)

        # Get magnitudes and convert to tensor
        mixture_mag = torch.from_numpy(np.abs(mixture_stft)).float().unsqueeze(0)
        drums_mag = torch.from_numpy(np.abs(drums_stft)).float().unsqueeze(0)

        return mixture_mag, drums_mag

def train(model, device, train_loader, optimizer, criterion):
    model.train()
    running_loss = 0.0
    for mixture, drums in tqdm(train_loader, desc="Training Drums Model"):
        mixture, drums = mixture.to(device), drums.to(device)

        optimizer.zero_grad()
        output = model(mixture)
        loss = criterion(output, drums)
        loss.backward()
        optimizer.step()
...
def main():
    ...
    train_dataset = MUSDBDrumsDataset(root_dir='musdb18/train', duration=2, samples_per_track=16)
    ...
    torch.save(model.state_dict(), 'checkpoints/unet_drums.pth')
    print("Drums separation model saved to checkpoints/unet_drums.pth")
